...
假设我们要设计一个工厂，用来管理各种重型对象（比如数据库连接、AI 模型、第三方客户端）。我们不想为每个类型都写一遍 `synchronized` 逻辑。
...
**需求背景：**
我们正在开发一个聚合支付系统。系统需要支持支付宝 (Alipay)、微信 (Wechat)、PayPal 等多种渠道。
每个渠道对应的 `PaymentClient` 初始化非常耗时，要加载证书、建立长连接，且必须是**单例**的。

**要求：**

1. 不要在系统启动时把所有 Client 都创建出来（太慢）。
2. 当第一个用户选择用支付宝支付时，才初始化支付宝 Client。
3. 后续用户再用支付宝，直接复用。
4. 请使用 RocketMQ 的这种 Function 设计模式实现。
...
class AlipayClient implements PaymentClient {
    public AlipayClient() { System.out.println("初始化支付宝客户端(加载证书耗时2秒)..."); }
    public void pay(double amount) { System.out.println("调用支付宝接口扣款: " + amount); }
}
...
public class PaymentService {
    public static void main(String[] args) {
        System.out.println("--- 用户A: 使用支付宝 ---");
        PaymentClient client1 = PaymentClientFactory.getClient("ALIPAY"); // 触发初始化
        client1.pay(100.0);
        ...
        System.out.println("\\n--- 用户C: 使用微信 ---");
        PaymentClient client3 = PaymentClientFactory.getClient("WECHAT"); // 触发初始化
        client3.pay(200.0);
    }
}
...
1. Key-Value 映射：需要根据某个 Key 获取对象。
2. 对象很重：创建对象开销大，不能每次都 new。
3. 按需加载：不希望启动时全部加载。
4. 线程安全：多线程环境下只能创建一个实例。

> 用 Map 做缓存，用 computeIfAbsent 做入口，用 Lambda (Function) 写创建逻辑。

这比写一大堆 `if (map.get(k) == null) { synchronized { ... } }` 要高级得多，代码可读性也更高。

RocketMQ 5.x 也是利用这一点，让 Factory 类变得非常简洁，可以轻松管理 Provider、Evaluator、Strategy 等多种类型的组件。
...
- **Consumer**：如果一个参数主要是用来**接收**数据，也就是我们要把数据塞给它，它就是消费者。
  - 比如  List<? super T>，你要往里  add  东西。

它体现了作者对 Java 泛型的深刻理解：**对输入宽容（接受子类），对输出严谨**。
